...
public class VueMain extends JPanel{
    ...
    private void chercherImage(CarteOrange carte) {
        if (carte.getTypeTresor() != null) {
            try {
                image = ImageIO.read(new File("DossierImage/AutreCarteJoueur/"+carte.getTypeTresor()+".png"));
            } catch (IOException ex) {
                Logger.getLogger(VueMain.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
            try {
                image = ImageIO.read(new File("DossierImage/AutreCarteJoueur/"+carte.getTypeClasse()+".png"));
            } catch (IOException ex) {
                Logger.getLogger(VueMain.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
